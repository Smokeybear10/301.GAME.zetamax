-- Zetamax v1 — per-round margin-aware ELO
-- Adds user_ratings + rating_events tables, the apply_run_elo() function
-- triggered from /api/runs/finish, and updates get_league_leaderboard to
-- surface the rating + provisional flag alongside the existing best/runs.
--
-- Asymmetric per-round update: only the player who finishes a round moves.
-- Their opponents update on their own next rounds. Margin-aware via tanh.
-- Provisional K=32 for first 30 rated rounds, K=16 thereafter, divided by
-- opponent count to bound per-round churn.
--
-- Idempotent: safe to re-run.

-- ============================================================================
-- user_ratings: one row per user, global rating across all leagues.
-- ============================================================================
...
-- ============================================================================
-- rating_events: per-round audit trail. The `details` jsonb stores the
-- per-opponent breakdown so we can re-render it later or debug.
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_rating_events_run
  ON public.rating_events (run_id);

-- ============================================================================
-- RLS — anyone authed can read ratings (leaderboards need them); rating_events
-- is private (own rows only). Writes are service-role only.
-- ============================================================================
...
-- ============================================================================
-- apply_run_elo(run_id): the per-round update. Called by /api/runs/finish
-- after a successful validation. Returns the rating delta + new rating +
-- per-opponent breakdown so the route handler can pass it back to the client.
--
-- Per-opponent deltas are rounded to int and summed; the total equals the
-- sum of rounded breakdown values exactly (no rounding skew). Rating is
-- floored at 1000.
-- ============================================================================
...
BEGIN
  ...
  -- No-op if the run doesn't qualify
  ...
  -- Make sure the runner has a rating row
  ...
  -- Count unique league mates who played today (best-of-day with score >= 5).
  -- Distinct because a friend might share multiple leagues with the runner.
  ...
  IF v_n = 0 THEN
    -- No qualifying opponents: refresh activity stamps but no delta, no
    -- matches_played increment (round didn't really count as rated).
  ...
  -- Iterate qualifying opponents and accumulate the delta + breakdown.
  FOR v_opp IN
    SELECT
  ...
  LOOP
    -- Standard ELO expected score from rating gap
    v_expected := 1.0 / (1.0 + power(10.0::numeric, (v_opp.opp_rating - v_runner_rating)::numeric / 400.0));
    -- Margin-aware actual: tanh maps score gap → smooth (0, 1)
...
-- ============================================================================
-- Re-create get_league_leaderboard with rating + peak + provisional columns.
-- Sort key changes: rating DESC primary, best_score DESC tie-break, earliest
-- started_at as final tie-break.
-- ============================================================================
