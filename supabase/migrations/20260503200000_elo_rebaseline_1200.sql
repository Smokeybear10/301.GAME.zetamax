-- ============================================================================
-- ELO rebaseline: shift the system anchor from 1500 to 1200.
--
-- - New players start at 1200 (was 1500).
-- - Existing players shift down by 300 (clamped at floor 800), preserving
--   relative ordering. A player at 1547 becomes 1247.
-- - Expected-score curve re-anchors so 1200 = expected score 35.
-- - Rating floor drops from 1000 to 800 (proportional to the base shift).
-- - Leaderboard COALESCE fallbacks update from 1500 to 1200.
-- ============================================================================
...
ALTER TABLE public.user_ratings
  ALTER COLUMN peak_rating SET DEFAULT 1200;

-- 2. Backfill existing rows: shift everyone down 300, clamped at the new floor
UPDATE public.user_ratings
SET rating      = GREATEST(800, rating - 300),
    peak_rating = GREATEST(800, peak_rating - 300);

-- ============================================================================
-- 3. apply_run_elo — re-create with 1200 anchor and 800 floor.
--    Math is identical to the v2 (race + baseline) version; only the
--    constants change.
-- ============================================================================
...
BEGIN
  ...
  THEN
    SELECT COALESCE(rating, 1200), COALESCE(matches_played, 0) < 30
      INTO v_runner_rating, v_provisional
    FROM public.user_ratings
    WHERE user_id = v_runner_id;
    RETURN QUERY SELECT
      0,
      COALESCE(v_runner_rating, 1200),
      0,
      COALESCE(v_provisional, true),
      '[]'::jsonb,
      0,
      GREATEST(0, 35 + (COALESCE(v_runner_rating, 1200) - 1200) / 25);
    RETURN;
  END IF;
  ...
  -- Baseline delta — anchored at 1200 = expected score 35.
  v_expected_score := GREATEST(0, 35 + (v_runner_rating - 1200) / 25);
  ...
  IF v_n > 0 THEN
    ...
    FOR v_opp IN
      SELECT
        ...
        COALESCE(MAX(ur.rating), 1200)        AS opp_rating,
        COALESCE(
  ...
  v_new_rating      := GREATEST(800, v_old_rating + v_total_delta_int);
...
-- ============================================================================
-- 4. get_league_leaderboard — re-create with 1200 fallback.
--    Same body as the daily migration version; only COALESCE constants change.
-- ============================================================================
